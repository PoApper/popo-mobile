...
vX.Y.Z 릴리즈 버전 범프. (Android versionCode N, iOS build M)
...
- (`fix:` 커밋을 "어떤 증상이 어떻게 고쳐졌는지"로 서술)

### 기타

- (`refactor:` `ci:` `build(deps):` `chore:` `test:`는 한 줄로 뭉침)

## 배포 전 테스트

- [ ] (시나리오: 무엇을 어떻게 조작하면 무엇이 나와야 하는지)

## 주의사항

- (회귀 위험이 있는 변경, 서버 버전 의존, 마이그레이션 필요 여부 등)
```
